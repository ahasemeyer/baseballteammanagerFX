...
public class Hitter implements Serializable 
{
    ...
    public static Hitter loadHitterData(int inPlayerID)
    {
        ...
        Query RBI = em.createNativeQuery("SELECT a.rbi FROM hitter a WHERE a.playerID=?");
        RBI.setParameter(1,inPlayerID);
        int inRBI = (int)RBI.getSingleResult();
        
        Hitter returnHitter = new Hitter(inPlayerID);
        returnHitter.updateHitter(inAB, inH, inB2, inB3, inHR, inBB, inHBP, inSac, inSO, inCS, inSB, inRBI);
    ...
    public void updateRBI(int inRBI)
    {
        this.rbi += inRBI; 
    }
